...
<link rel="stylesheet" type="text/css" href="./css/header2.css?v=3">
<link rel="stylesheet" type="text/css" href="./css/default.css?v=1">
<link rel="stylesheet" type="text/css" href="./css/part3.css?v=7">
<script src="./js/part3.js?v=3" defer></script>
</head>
<body>
<header>
<div class="top">
...
<form id="f">
<section>
<input type="hidden" value="<%=request.getAttribute("sarea1") %>" name="sarea1">
<input type="hidden" value="<%=request.getAttribute("sarea2") %>" name="sarea2">
<input type="hidden" value="<%=request.getAttribute("sdate1") %>" name="sdate1">
<input type="hidden" value="<%=request.getAttribute("sdate2") %>" name="sdate2">
<input type="hidden" value="<%=request.getAttribute("slocal1") %>" name="slocal1">
<input type="hidden" value="<%=(String)request.getAttribute("sno") %>" name="sno">

<input type="hidden" value="<%=request.getAttribute("stotal") %>" name="stotal">
<input type="hidden" value="<%=request.getAttribute("sname") %>" name="sname">
<input type="hidden" value="<%=request.getAttribute("stel") %>" name="stel">
<input type="hidden" value="<%=request.getAttribute("sle") %>" name="sle">
<input type="hidden" value="" name="pay">
	<div class="all">
		<p>야영장 예약하기</p>
		<div><img src="./ico/camping.svg" class="ico"><span><%=(String)request.getAttribute("sarea1")%> <%=(String)request.getAttribute("slocal1")%> 야영장</span><span><%=(String)request.getAttribute("sdate1")%> ~ <%=(String)request.getAttribute("sdate2")%></span></div>
		<div><span>야영장 위치</span><span><%=(String)request.getAttribute("sarea2")%> - <%=(String)request.getAttribute("slocal1")%></span></div>
		<div>총 금액: <%=(String)request.getAttribute("stotal")%>원</div>
		<div><img src="./ico/report.svg" class="ico"><span>예약자 정보</span></div>
		<div><span>고객명</span><span><%=(String)request.getAttribute("sname")%></span></div>
		<div><span>연락처</span><span><%=(String)request.getAttribute("stel")%></span></div>
		<div><span>인원수</span><span><%=(String)request.getAttribute("sle")%>명</span></div>
		<div><span>예약번호</span><span><%=(String)request.getAttribute("sno")%></span></div>
		<div><img src="./ico/pay.svg" class="ico"><span>결제수단<span></div>
		<div class="box_all">
			<span class="box" id="pay" onclick="pay(1)">무통장 입금</span>
			<span class="box" id="card" onclick="pay(2)">신용카드</span>
			<span class="box" id="bank" onclick="pay(3)">계좌이체</span>
		</div>
		<div class="pay" id="pay2">신한은행 : 102-202306-0606 (국립공원공단) <input type="text" placeholder="입금자명을 입력"></div>
		
		<input type="button" class="btn" value="결제 선택" onclick="gogo()">
	</div>
</section>
</form>
	<footer>
